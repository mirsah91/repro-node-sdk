...
// ====== NEW: tiny, safe tracer auto-init (no node_modules, no patches) ======
type TracerApi = {
    init?: (opts: any) => void;
...
(function ensureTracerAutoInit() {
    ...
    try {
        // 1) load your bundled tracer (kept inside this package)
        const tracerPkg: TracerApi = require('../tracer');

        const cwd = process.cwd().replace(/\\/g, '/');
        const sdkRoot = __dirname.replace(/\\/g, '/');

        // 2) include ONLY app code (no node_modules) to avoid interfering with deps
        const include = [ new RegExp('^' + escapeRx(cwd) + '/(?!node_modules/)') ];

        // 3) exclude this SDK itself (and any babel internals if present)
        const exclude = [
        ...
        // make sure per-request traceId exists; harmless if init already did it
        tracerPkg.patchHttp?.();

        __TRACER__ = tracerPkg;
        __TRACER_READY = true;
    } catch {
        // tracer not bundled or optional — we keep functioning without it
        __TRACER__ = null;
    }
})();
// ===========================================================================
...
export function reproMiddleware(cfg: { appId: string; appSecret: string; apiBase: string }) {
    return function (req: Request, res: Response, next: NextFunction) {
        ...
        // ---- our ALS (unchanged) ----
        als.run({ sid, aid }, () => {
            // -------- NEW: subscribe to tracer for this request (passive) --------
...
// ===================================================================
// reproMongoosePlugin — your stable implementation (unchanged)
// ===================================================================
                ...
                Q.exec = async function patchedExec(this: any, ...args: any[]) {
                    ...
                    try {
                        ...
                        if (sid) emitDbQuery(cfg, sid, aid, {
                            collection, op, query: { filter, update, projection, options },
